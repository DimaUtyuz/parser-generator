...
public class ExpressionParser {
    ...
    Tree<Function<Double, Double>> start() throws ParseException {
        if (Set.of(ExpressionToken.NUMBER, ExpressionToken.OPEN_BR).contains(lex.curToken())) {
    ...
    Tree<Function<Double, Double>> ee() throws ParseException {
        if (Set.of(ExpressionToken.END, ExpressionToken.CLOSE_BR).contains(lex.curToken())) {
    ...
    Tree<Function<Double, Double>> tt() throws ParseException {
        if (Set.of(ExpressionToken.ADD, ExpressionToken.SUB, ExpressionToken.END, ExpressionToken.CLOSE_BR).contains(lex.curToken())) {
            Tree<Function<Double, Double>> result = new Tree<>("eps");
            result.setValue(a -> a);
            return result;
    ...
    Tree<Function<Double, Double>> rr() throws ParseException {
        if (Set.of(ExpressionToken.DIV, ExpressionToken.ADD, ExpressionToken.SUB, ExpressionToken.MUL, ExpressionToken.END, ExpressionToken.CLOSE_BR).contains(lex.curToken())) {
            Tree<Function<Double, Double>> result = new Tree<>("eps");
            result.setValue(a -> a);
            return result;
    ...
    Tree<Function<Double, Double>> f() throws ParseException {
        if (Set.of(ExpressionToken.NUMBER).contains(lex.curToken())) {
            ...
            return result;
        } else if (Set.of(ExpressionToken.OPEN_BR).contains(lex.curToken())) {
            if (lex.curToken() != ExpressionToken.OPEN_BR) {
                throw new ParseException("Expected another token at position " + lex.curPos(), lex.curPos());
            }
            Tree<Function<Double, Double>> OPEN_BR = new Tree<>("OPEN_BR");
            OPEN_BR.setValue(null);
            lex.nextToken();
            Tree<Function<Double, Double>> start = start();
            if (lex.curToken() != ExpressionToken.CLOSE_BR) {
                throw new ParseException("Expected another token at position " + lex.curPos(), lex.curPos());
            }
            Tree<Function<Double, Double>> CLOSE_BR = new Tree<>("CLOSE_BR");
            CLOSE_BR.setValue(null);
            lex.nextToken();
            Tree<Function<Double, Double>> result = new Tree<>("f", OPEN_BR, start, CLOSE_BR);
            result.setValue(a -> start.getValue().apply(0D));
            return result;
        }
        throw new ParseException("Expected another token at position " + lex.curPos(), lex.curPos());
    }
